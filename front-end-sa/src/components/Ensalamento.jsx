...
const Ensalamento = () => {
    ...
    return (
        <div className='container'>
            ...
            {/* escolher curso */}
            <select value={curso} onChange={handleCurso}>
                <option value="">Escolher Curso</option>
                <option value="Desenvolvimento">Desenvolvimento</option>
            </select>

            {/* escolher fase */}
            <select value={fase} onChange={handleFase}>
            ...
            {(fase && curso) && (
                <>
                    <ul className='container-lista lista-scroll' style={{display:"flex", gap: '20px'}}>
                        <div className='week-day'>
                            <h2>SEG</h2>
                            <div>
                                <button className='botao-cadastrar' onClick={openModal}>Ensalar</button>
                            </div>
                        </div>
                        <div className='week-day'>
                            <h2>TER</h2>
                    ...
                    <div className="modal">
                        ...
                        <select >
                            ...
                            {Object.values(professores.professoresLista || {}).map(professor => (
                                <option value={professor.nome} key={professor.id_prof}>{professor.nome}</option>))
                            }
                        </select>
                        )}
                        ...
                        <select >
                            ...
                            {Object.values(disciplinas.disciplinasLista || {}).map(disciplina => (
                                <option value={disciplina.nm_disciplina} key={disciplina.id_discip}>{disciplina.nm_disciplina}</option>))
                            }
                        </select>
                        )}
                        ...
                        <select >
                            ...
                            {Object.values(salas.salasLista || {}).map(sala => (
                                <option value={sala.num_sala} key={sala.id_sala}>{sala.num_sala}</option>))
                            }
                        </select>
                        )}
                        <div className='button-grupo-modal'>
                            <button onClick={closeModal} className="botao-cadastrar">Salvar</button>
